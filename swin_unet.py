...
class WindowAttention(nn.Module):
    ...
    def window_partition(self, x: torch.Tensor) -> torch.Tensor:
        ...
        #print(1111111111, x.shape, self.window_size)
...
from ..models.FrANlayer import ChirpFANLayer

class SwinTransformerBlock(nn.Module):
    def __init__(self, dim, num_heads, window_size=7, shift=False, mlp_ratio=4., 
                 p_ratio=0.25, qkv_bias=True, drop=0., attn_drop=0., drop_path=0., 
...
class BasicBlock(nn.Module):
    def __init__(self, index: int, embed_dim: int = 96, window_size: int = 7, depths: tuple = (2, 2, 6, 2),
                 num_heads: tuple = (3, 6, 12, 24), mlp_ratio: float = 4., p_ratio:float =0.25,qkv_bias: bool = True,
    ...
    def __init__(self, index: int, embed_dim: int = 96, window_size: int = 7, depths: tuple = (2, 2, 6, 2),
        ...
        self.blocks = nn.ModuleList([
            SwinTransformerBlock(
                ...
                num_heads=num_head,
                window_size=window_size,
                shift=False if (i % 2 == 0) else True,#### W-MSA和SW-MSA 交替使用 
                                                      # W-MSA：将全局注意力变为局部注意力，降低复杂度；但块间缺少信息交流
                                                      ## SW-MSA： 实现块间信息交流，全局注意力
                mlp_ratio=mlp_ratio,
                qkv_bias=qkv_bias,
                drop=drop_rate,
                attn_drop=attn_drop_rate,
...
class SwinUnet(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.first_patch_expanding(x)## pathc 从7x7 扩充为14x14
